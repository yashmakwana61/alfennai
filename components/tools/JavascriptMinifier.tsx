...
import type { JavascriptMinifierInput, JavascriptMinifierOutput } from "@/config/tools/javascript-minifier.config";
...
export function JavascriptMinifier({ tool }: { tool: ToolConfig<JavascriptMinifierInput, JavascriptMinifierOutput> }) {
  const [raw, setRaw] = useState("");
  const [result, setResult] = useState<JavascriptMinifierOutput | null>(null);
  ...
  return (
    <div>
      <FieldLabel htmlFor="raw">JavaScript input</FieldLabel>
      <TextArea id="raw" rows={8} value={raw} onChange={(e) => setRaw(e.target.value)} placeholder="function add(a, b) { return a + b; }" />
      <ErrorText>{error}</ErrorText>
      <div className="mt-4 flex flex-wrap gap-3">
        <PrimaryButton onClick={run}>Minify JavaScript</PrimaryButton>
